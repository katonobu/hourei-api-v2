...
# ワーク・ライフ・バランスの実現に向けた10の実践
work_life_balance_objs = [
    {
        "title": "1.会議のムダ取り",
        "sentences": [
...
def main():
    ...
    album_name = "コンピテンシー"
    album_count = local_convert_to_mp3(
        base_dir, artist, album_name, competency_objs, album_count=album_count, dry_run=dry_run)
    album_name = "技術士倫理綱領"
    album_count = local_convert_to_mp3(
        base_dir, artist, album_name, kouryou_objs, album_count=album_count+1, dry_run=dry_run)
    album_name = "ワーク・ライフ・バランスの実現に向けた10の実践"
    album_count = local_convert_to_mp3(
        base_dir, artist, album_name, work_life_balance_objs, album_count=album_count+1, dry_run=dry_run)
